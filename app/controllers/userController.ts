...
import { insufficientParameters, mongoError, successResponse, failureResponse } from '../modules/common/modules.common.service';
import { IUser } from '../modules/user/modules.user.model';
...
export class UserController {
    ...
    public get_all_users(req: Request, res: Response) {
        this.user_service.getAllUsers((err: any, user_data: IUser[]) => {
            if (err) {
                mongoError(err, res);
            } else {
                successResponse('get all users successfull', user_data, res);
            }
        });
    }
